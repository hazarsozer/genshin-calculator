...
const CATEGORY_ORDER = ["attack", "skill", "burst", "reaction", "heal", "shield"];
...
function groupFeatures(feats: readonly FeatureResult[]): [string, Node[]][] {
  ...
  for (const child of feats) {
    ...
    for (const parent of feats) {
      if (parent.key === child.key || catOf(parent.key) !== catOf(child.key)) continue;
                ...
                <div key={node.feat.key}>
                  ...
                  {node.children.length > 0 && (
                    <div className="mt-1.5 ml-3 flex flex-col gap-1 border-l border-[var(--ck-border)] pl-3">
                      {node.children.map((c) => (
                        <div key={c.key} className="flex items-baseline justify-between text-[var(--ck-muted)]">
                          <span className="text-[11.5px]">{c.label}</span>
                          <span className="text-[12px] tabular-nums">{fmt(c.triple[mode])}</span>
                        </div>
                      ))}
                    </div>
                  )}
